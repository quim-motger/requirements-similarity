...
@Service
public class StanfordPreprocessService {
    ...
    public List<Token> getUniqueTokens(Annotation requirement) {
        List<Token> tokenList = new ArrayList<>();
        List<CoreMap> qssTokens = requirement.get(CoreAnnotations.SentencesAnnotation.class);
        for (CoreMap sentenceTokens : qssTokens) {
            ...
            for (CoreLabel t : tokens) {
                String lemma = t.lemma().toLowerCase();
                String pos = t.tag();
                if (!Arrays.asList(stopwords).contains(lemma) && !tokenList.contains(lemma)) {
                    tokenList.add(new Token(lemma, pos));
                }
            }
        }
        return tokenList;
    ...
    public List<String> getNGrams(Annotation requirement, int n) {
        List<String> ngrams = new ArrayList<>();
        List<CoreMap> qssTokens = requirement.get(CoreAnnotations.SentencesAnnotation.class);
        for (CoreMap sentenceTokens : qssTokens) {
            ArrayList<CoreLabel> tokens = (ArrayList<CoreLabel>) sentenceTokens.get(CoreAnnotations.TokensAnnotation.class);
            List<String> unigrams = tokens.stream().map(CoreLabel::lemma).filter(s -> !Arrays.asList(stopwords).contains(s)).collect(Collectors.toList());
