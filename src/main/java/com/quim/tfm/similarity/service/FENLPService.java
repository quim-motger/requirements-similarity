...
public class FENLPService {
    ...
    public FEPreprocessData applyFEPreprocess(String[] tokens) throws IOException {
        ...
        String[] lemmas = Arrays.stream(lemmatizer.lemmatize(tokens, posTags)).filter(s -> !s.equals("O")).collect(Collectors.toList()).toArray(new String[0]);
    ...
    private DependencyTreeNode extractDependencies(String text) {
        return null;
    }
